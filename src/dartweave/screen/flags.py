...
VERIFICATION: dict[str, str] = {
    ...
    "이자보상배율 1 미만": (
        "**채택** · 기준시점 2개(2023·2024) × 규모·업종 통제 7설정 = 14개 조합 **전부**에서 유의 (최보수 ×4.23 / ×5.78)"
        "· 해당 714~901사 · 실측 부실률 3.6%~5.4%"
    ),
    "영업현금흐름 음수": (
        "**채택** · 기준시점 2개(2023·2024) × 규모·업종 통제 7설정 = 14개 조합 **전부**에서 유의 (최보수 ×3.34 / ×4.75)"
        "· 해당 764~810사 · 실측 부실률 3.1%~6.0%"
    ),
    "결손금": (
        "**채택** · 기준시점 2개(2023·2024) × 규모·업종 통제 7설정 = 14개 조합 **전부**에서 유의 (최보수 ×5.32 / ×8.86)"
        "· 해당 744~759사 · 실측 부실률 3.8%~7.0% · 두 시점 전부 Bonferroni 통과"
    ),
    "당기순손실": (
        "**채택** · 기준시점 2개(2023·2024) × 규모·업종 통제 7설정 = 14개 조합 **전부**에서 유의 (최보수 ×7.94 / ×11.12)"
        "· 해당 1,027~912사 · 실측 부실률 3.5%~5.6%"
    ),
    "영업손실": (
        "**채택** · 기준시점 2개(2023·2024) × 규모·업종 통제 7설정 = 14개 조합 **전부**에서 유의 (최보수 ×6.75 / ×7.19)"
        "· 해당 763~949사 · 실측 부실률 3.8%~5.5%"
    ),
    "이익잉여금 3년 악화": (
        "**채택** · 기준시점 2개(2023·2024) × 규모·업종 통제 7설정 = 14개 조합 **전부**에서 유의 (최보수 ×5.34 / ×7.57)"
        "· 해당 501~614사 · 실측 부실률 4.6%~8.1% · 같은 개수 안에서도 가른다 — 4개 걸림 + 악화 231건 5.6% 대 악화 아님"
        "161건 1.9%(두 시점 합산)"
    ),
    "영업이익 3년 악화": (
...
# 같은 개수 안에서 이익잉여금 방향이 가르는 정도.
#
# **두 기준시점을 (회사, 시점) 쌍으로 합쳐 잰다.** 한 시점만 쓰면 칸이 80~130사로
# 얇아져 답이 흔들린다 — 실측으로 T=2024 의 "4개 + 악화 7.1% 대 악화 아님 0.0%" 가
# T=2023 에서는 3.8% 대 3.8% 로 차이가 사라졌다. 합치면 231쌍 5.6% 대 161쌍 1.9% 로
# 두 시점 모두를 반영한 값이 나온다. 칸당 20건 규율을 여기에도 건다.
#
#   3개   악화 181쌍  5.5%  ·  악화 아님 208쌍  1.0%
#   4개   악화 231쌍  5.6%  ·  악화 아님 161쌍  1.9%
#   5개   악화 445쌍  9.9%  ·  악화 아님  88쌍  5.7%
...
def direction_split(fired: int, worsening: bool | None) -> str:
    ...
    if worsening:
        return (f"게다가 **누적 손실이 3년째 커지는 중**입니다. 같은 {tag} 걸렸고 "
                f"방향도 악화였던 {n_bad}건 중 {r_bad:.1f}%가 부실로 갔습니다 — "
                f"방향이 악화가 아니었던 {n_ok}건은 {r_ok:.1f}%였습니다 "
                f"(기준시점 2개 합산).")
    return (f"다만 **누적 손실이 커지는 중은 아닙니다.** 같은 {tag} 걸렸는데 "
            f"방향이 악화가 아니었던 {n_ok}건의 부실률은 {r_ok:.1f}%로, "
            f"악화였던 {n_bad}건({r_bad:.1f}%)보다 낮습니다 (기준시점 2개 합산).")
...
_AUDIT_IN_DEFICIT: dict[str, tuple[int, float, str]] = {
    "adverse": (79, 30.4, "의견거절·한정"),
    "concern": (69, 13.0, "계속기업 경고"),
    "none": (1160, 3.7, "감사 경고 없음"),
}
# 신호 5개 구간에서는 훨씬 크게 갈린다 — 여기가 이 층의 값어치다.
_AUDIT_AT_FIVE = (478, 5.0, 82, 30.5)
# 그리고 둘 다 깨끗하면 실측 부실 0건이었다.
_AUDIT_CLEAN_ZERO = (2178, 1)
...
def audit_split(fired: int, audit: str | None, year: str | None = None) -> str:
    ...
    if audit == "none" and fired == 0:
        ...
        return (f"{stamp}감사인도 아무 경고를 달지 않았습니다. "
                f"**신호 0개 + 감사 경고 없음**"
                f"이었던 {n:,}건 중 이후 2년 안에 부실로 간 곳은 "
                f"**{rate:.0f}건**이었습니다 (기준시점 2개 합산).")
    if fired >= 5:
        n_ok, r_ok, n_bad, r_bad = _AUDIT_AT_FIVE
        if audit == "none":
            return (f"{stamp}**감사인은 경고를 달지 않았습니다.** 같은 5개 걸렸는데 "
                    f"감사 경고가 없던 {n_ok}건의 부실률은 {r_ok:.1f}%로, 경고가 "
                    f"있던 {n_bad}건({r_bad:.1f}%)의 6분의 1이었습니다 (기준시점 2개 합산).")
        n, rate, label = _AUDIT_IN_DEFICIT[audit]
        marked = _subject(label).replace(label, f"**{label}**", 1)
        return (f"{stamp}{marked} 있습니다. 같은 5개 걸렸고 감사 "
                f"경고까지 있던 {n_bad}건 중 {r_bad:.1f}%가 부실로 갔습니다 — 경고가 "
                f"없던 {n_ok}건은 {r_ok:.1f}%였습니다. 감사 경고 자체를 규모·업종 "
                f"통제 7설정에 걸면 **최보수 ×16.33**으로, 이 저장소가 잰 것 중 가장 "
                f"강합니다 (다만 두 기준시점 중 한 시점만 판정이 났습니다).")
    n, rate, label = _AUDIT_IN_DEFICIT[audit]
    if audit == "none":
        return ""          # 신호가 적고 경고도 없으면 더 할 말이 없다
    marked = _subject(label).replace(label, f"**{label}**", 1)
    none_n, none_rate, _ = _AUDIT_IN_DEFICIT["none"]
    return (f"{stamp}{marked} 있습니다. 결손금이 있으면서 같은 표기를 "
            f"받았던 {n}건 중 {rate:.1f}%가 이후 2년 안에 부실로 갔습니다 — 결손금만 "
            f"있고 감사 경고는 없던 {none_n:,}건은 {none_rate:.1f}%였습니다 "
            f"(기준시점 2개 합산).")
...
# 누계벌점 — 이진을 등급으로 바꾼다. 벌점 1점짜리 한 번과 15점은 같은 사건이 아니다.
#
# **8점은 우리가 고른 값이 아니다** — 코스닥 공시규정의 관리종목 지정선이다. 그런데
# 여기가 이 도구가 규정을 베끼지 않고 **검증하는** 거의 유일한 자리이기도 하다.
# 지정 1,021건의 공시 본문을 전부 받아 재보니 실측이 규정선과 맞았다(두 시점 단조):
#
#            T=2023               T=2024
...
#     71%(725/1,021)라 8점 선은 사실상 코스닥 기준이고, 코스닥 안에서 잘 갈린다
#     (8점 미만 3.7%/12.2% 대 8점 이상 26.3%/31.6%). 유가증권은 8점 이상이 6사뿐이라
#     못 가른다.
#   · **대체부과 벌점을 복원해서 쓰면 안 된다.** 코스닥은 벌점 대신 제재금을 물릴 수
#     있고(231건), 그러면 본문의 부과벌점이 0.0 으로 찍힌다. 처음엔 제재금 ÷ 400만원으로
#     실질 벌점을 복원해 쓰려 했는데, 재보니 **거래소가 가볍게 봤다는 것 자체가 정보**였다:
#
#       대체부과만 있던 회사      35/44사   5.7% / 9.1%  (×3.9)
#       대체부과가 없던 회사     126/110사  9.5% / 15.5% (×6.5)
#
#     복원해서 같은 자에 놓으면 이 회사들이 실제보다 위험한 쪽으로 올라간다. 그래서
#     신호는 **본문에 적힌 누계벌점 그대로** 쓴다(대체부과 회사는 자연히 0점 근처에
#     모인다 — 중앙값 0, 35사 중 27사가 0점). `effective` 는 CSV 에만 남기고 안 쓴다.
#
# ⚠️ **좁은 답안지(부실만)에서는 검정을 통과하지 못했다.** 해당 44~89사 중 실제 부실이
#    11~19건이라 최소 20건에 못 미친다. 위 배율은 통제 전 값이라 그렇게 부르면 안 된다.
#
#    **넓은 답안지(부실+관리종목)에서는 4점 선이 통과했다** — 두 시점 전부 채택 ·
#    최보수 ×6.60~6.96 · 해당 80~89사에 부실률 31.5~40.0%. 8점 선은 해당 44사로
#    표본이 얇아 아직 판정 1/2 다. 그래서 화면에는 관측 형태로 내되, 4점 선이 통제를
#    견뎠다는 사실은 따로 적는다.
#
# **칸당 사례가 20건에 미달하면 순서를 말하지 않는다.** 검정 쪽은 이 규칙을 갖고
# 있었는데(신호군 부실 20건 미만 = 판정 불가) 빠르게 재는 쪽에는 없어서, 260건 표본에서
# "0~2점 10사 부실 0건" 과 "대체부과 61사 부실 1건" 을 각각 0.0% 와 1.6% 로 읽고
# "규정선이 틀렸다 · 열네 배" 라는 결론을 냈다. 1/61 은 한 건만 더 있어도 두 배가 된다 —
# 비율이 아니라 잡음이었다. 전량 1,021건에서 둘 다 뒤집혔다.
#
# 걸린 개수 안에서 다시 벌점으로 쪼개지는 않는다 — 칸마다 20사 미만이 되어 없는
# 정밀도를 만들게 된다. 개수 안 좁혀주기는 이력 유무(이진)로 하고, 벌점은 전체
# 구간으로 따로 낸다.
_PENALTY_BANDS: tuple[tuple[float, float, str, int, float], ...] = (
    (0.0, 4.0, "0~4점", 202, 4.0),
    (4.0, 8.0, "4~8점", 87, 11.5),
    (8.0, float("inf"), "8점 이상", 97, 27.8),
)
PENALTY_LINE = 8.0          # 코스닥 공시규정 관리종목 지정선. 우리가 고른 값이 아니다.


def penalty_band(points: float | None) -> str:
    """누계벌점이 어느 구간인가. None 이면 빈 문자열 — 모르는 걸 0 점으로 세지 않는다.

    0 점도 빈 문자열이다. 불성실공시로 지정된 적이 없다는 뜻이고, 그건 이미
    `disclosure_split` 이 말한다 — 같은 사실을 두 번 말하지 않는다.
    """
    if not points:
        return ""
    for lo, hi, name, n, rate in _PENALTY_BANDS:
        if lo <= points < hi:
            tail = ("" if lo < PENALTY_LINE else
                    " — **코스닥 공시규정의 관리종목 지정선**입니다. 우리가 고른 "
                    "값이 아니고, 실측이 이 선과 맞았습니다")
            return (f"최근 3년 누계벌점이 **{points:g}점**으로 **{name}** 구간입니다. "
                    f"같은 구간이었던 {n}건 중 {rate:.1f}%가 이후 2년 안에 "
                    f"부실로 갔습니다{tail}. (관측 — 다만 관리종목까지 포함한 답안지로는 "
                    f"**4점 선이 규모·업종 통제를 견뎠습니다**: 두 시점 전부 채택 · "
                    f"최보수 ×6.32~6.86.)")
    return ""
...
def disclosure_split(fired: int, count: int | None) -> str:
    ...
    body = (f"게다가 최근 3년에 **불성실공시 법인으로 {count}회 지정**됐습니다. 같은 "
            f"{tag} 걸렸고 불성실공시 이력도 있던 {n_bad}건 중 {r_bad:.1f}%가 부실로 "
            f"갔습니다 — 이력이 없던 {n_ok}건은 {r_ok:.1f}%였습니다 (기준시점 2개 합산).")
    if count >= 2:
