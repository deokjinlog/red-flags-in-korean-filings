...
VERIFICATION: dict[str, str] = {
    "결손금": (
        "**채택** · 기준시점 3개 × 규모·업종 통제 설정 7개 = 21개 조합 **전부**에서 유의 "
        "(최보수 ×6.35 / ×3.80 / ×2.76) · 다중검정 보정(가족 21)에서 "
        "**세 시점 전부 Bonferroni 통과** — 이 저장소에서 가장 강한 신호다"
    ),
    "당기순손실": (
        "**채택** · 21개 조합 전부에서 유의 (최보수 ×3.21 / ×2.02 / ×2.78) · 다중검정은 "
        "세 시점 전부 FDR 통과 · Bonferroni 는 3개 중 2개"
    ),
    "영업손실": (
        "**채택(약)** · 21개 조합 전부에서 유의 (최보수 ×2.24 / ×2.83 / ×2.02) 이지만 "
        "다중검정에서 **Bonferroni 를 넘는 시점이 3개 중 1개뿐**(임계 0.00238). "
        "결손금·당기순손실보다 한 단계 약한 근거다"
    ),
    "공동의존점 근접":
    ...
    "대기업집단과의 거리":
        "**채택 안 함** · 감사의견 라벨은 통제하면 ×2.61→×1.29 로 사라지고, 실제 부실 "
        "사건으로 앞을 보면 기준시점에 따라 **방향이 뒤집힌다** (2022 ×0.88 / 2023 ×1.70) — "
...
def accumulated_deficit(retained: float | None, *, year: str = "") -> Flag | None:
    ...
    **가장 강한 신호다.** 지분 구조 검사 7종은 교란을 통제하면 전부 사라지는데,
    이건 기준시점을 바꾸고 통제를 촘촘히 해도 남는다:
      ...
      T=2023-06-30 · 2022년 재무   해당 583사 · 이후 2년 부실 4.80%   ×2.76 · p=0.0005
    ...
    **배율을 "망한다" 로 읽으면 안 된다.** 기저율이 2.6~3.1% 라 배율을 곱해도 5~7% 다
    ...
    return Flag(
        ...
        evidence=["결손 기업의 이후 2년 부실률 실측 4.8~7.2% "
                  "(전체 2.6~3.1% · 규모·업종 통제 후 ×2.8~6.4 · 기준시점 3개)",
                  "다만 **결손 기업의 93~95%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 26%가 여기 걸리고, 실제 부실의 48~67%를 잡는다"],
    )
...
def operating_loss(op_income: float | None, *, year: str = "") -> Flag | None:
    ...
    **결손금보다 한 단계 약한 근거다.** 판정이 나온 검정 14개를 한 가족으로 묶어
    ...
    return Flag(
        ...
        evidence=["영업손실 기업의 이후 2년 부실률 실측 4.2~6.2% "
                  "(전체 2.6~3.1% · 규모·업종 통제 후 ×2.0~2.8 · 기준시점 3개)",
                  "다만 **영업손실 기업의 94~96%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 27%가 여기 걸리고, 실제 부실의 43~54%를 잡는다"],
    )
...
def net_loss(net_income: float | None, *, year: str = "") -> Flag | None:
    ...
    기준시점 3개 × 통제 설정 7개 전부에서 유의했다:
      ...
      T=2021-06-30   해당 727사 · 이후 2년 부실 5.09%   ×3.21 · p=0.0005
      T=2022-06-30   해당 646사 · 이후 2년 부실 5.07%   ×2.02 · p=0.0052
      T=2023-06-30   해당 713사 · 이후 2년 부실 4.47%   ×2.78 · p=0.0005
    ...
    **한때 '시점에 따라 갈린다' 며 뺐던 신호다.** 그 판정이 틀렸다 — 당시 재무 검정이
    ...
    셋 중 가장 넓게 걸린다(상장사의 30~38%). 재현율은 가장 높고(49~69%) 정밀도는
    ...
    return Flag(
        ...
        evidence=["순손실 기업의 이후 2년 부실률 실측 4.5~5.1% "
                  "(전체 2.6~3.1% · 규모·업종 통제 후 ×2.0~3.2 · 기준시점 3개)",
                  "다만 **순손실 기업의 95~96%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 32%가 여기 걸리고, 실제 부실의 49~69%를 잡는다"],
    )
