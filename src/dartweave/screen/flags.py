...
#
# ⚠️ **검정은 통과하지 못했다 — 관측이다.** 같은 파이프라인(규모·업종 통제 7설정 ×
#    순열검정)에 태웠더니 두 기준시점 다 판정 불가다. 해당 44~89사인데 그중 실제
#    부실이 11~19건이라 최소 20건에 못 미친다. 배율(×13~17)이 커 보여도 통제 전
#    값이므로 그렇게 부르면 안 된다. 화면에도 "같은 구간이었던 N사 중 X%" 라는
#    관측 형태로만 낸다.
#
    ...
    for lo, hi, name, n, r_lo, r_hi in _PENALTY_BANDS:
        if lo <= points < hi:
            ...
            return (f"최근 3년 누계벌점이 **{points:g}점**으로 **{name}** 구간입니다. "
                    ...
                    f"부실로 갔습니다{tail}. (관측 — 해당 기업이 적어 통제 검정은 "
                    f"판정이 나지 않았습니다.)")
    return ""
