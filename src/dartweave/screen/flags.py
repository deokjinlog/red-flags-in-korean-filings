...
VERIFICATION: dict[str, str] = {
    ...
    "대기업집단과의 거리":
        "검정됨 · 고립 6.8% vs 나머지 2.6% — 자산총계 4층 통제 후 **×1.69 · p=0.021** "
        "(단순 ×2.61 중 절반가량이 규모였다) — "
...
def conglomerate_distance(
    ...
    *,
    ...
    검정 결과(그래프 안 상장사 2,255사 · 감사의견 라벨 · 자산총계 확보분):
      통제 전   소속 1.6% → 미소속·연결 3.5% → 미소속·고립 6.8%
      규모 통제 후  고립 ×1.69 · p=0.021  (자산총계 4층 · 층 안에서만 순열)

    **가까울수록 문제가 적다.** 그래서 걸리는 건 '소속' 이 아니라 **'고립'** 이다 —
    이름을 반대로 읽지 않도록 검정 결과를 요약에 싣는다.

    배율을 통제 전 값으로 쓰면 안 된다. 고립군은 작은 회사에 몰려 있고 작은 회사는
    원래 문제가 많다. 자산총계로 층을 나눠 같은 규모끼리 비교하면 ×2.61 이 ×1.69 로
    줄어든다 — **겉보기 차이의 절반가량이 규모였다.** 남은 ×1.69 가 신호다.
