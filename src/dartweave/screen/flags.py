...
# 구간으로 따로 낸다.
_PENALTY_BANDS: tuple[tuple[float, float, str, int, float, float], ...] = (
    (0.0, 4.0, "0~4점", 94, 2.2, 6.4),
    (4.0, 8.0, "4~8점", 36, 8.9, 13.9),
    (8.0, float("inf"), "8점 이상", 44, 25.0, 31.8),
)
...
def penalty_band(points: float | None) -> str:
    ...
    if not points:
        return ""
    for lo, hi, name, n, r_lo, r_hi in _PENALTY_BANDS:
        if lo <= points < hi:
            ...
            return (f"최근 3년 누계벌점이 **{points:g}점**으로 **{name}** 구간입니다. "
                    f"같은 구간이었던 {n}사 중 {r_lo:.1f}~{r_hi:.1f}%가 이후 2년 안에 "
