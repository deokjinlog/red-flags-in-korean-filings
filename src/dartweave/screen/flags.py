...
def accumulated_deficit(retained: float | None, *, year: str = "") -> Flag | None:
    ...
    **배율을 "망한다" 로 읽으면 안 된다.** 기저율이 2.6~3.1% 라 배율을 곱해도 5~7% 다
    ...
    return Flag(
        ...
        evidence=["결손 기업의 이후 2년 부실률 실측 6.7~9.6% "
                  "(전체 2.9~4.1% · 규모·업종 통제 후 ×2.7~6.8 · 기준시점 4개)",
                  "다만 **결손 기업의 90~93%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 26%가 여기 걸리고, 실제 부실의 50~68%를 잡는다"],
    )
...
def operating_loss(op_income: float | None, *, year: str = "") -> Flag | None:
    ...
    **결손금보다 한 단계 약한 근거다.** 판정이 나온 검정 14개를 한 가족으로 묶어
    ...
    return Flag(
        ...
        evidence=["영업손실 기업의 이후 2년 부실률 실측 5.6~7.8% "
                  "(전체 2.9~4.1% · 규모·업종 통제 후 ×2.0~3.1 · 기준시점 4개)",
                  "다만 **영업손실 기업의 92~94%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 26~33%가 여기 걸리고, 실제 부실의 45~64%를 잡는다"],
    )
...
def net_loss(net_income: float | None, *, year: str = "") -> Flag | None:
    ...
    **한때 '시점에 따라 갈린다' 며 뺐던 신호다.** 그 판정이 틀렸다 — 당시 재무 검정이
    ...
    return Flag(
        ...
        evidence=["순손실 기업의 이후 2년 부실률 실측 5.5~7.9% "
                  "(전체 2.9~4.1% · 규모·업종 통제 후 ×2.3~4.0 · 기준시점 4개)",
                  "다만 **순손실 기업의 92~95%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 30~38%가 여기 걸리고, 실제 부실의 51~70%를 잡는다"],
    )
