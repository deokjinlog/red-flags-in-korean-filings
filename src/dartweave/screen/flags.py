...
VERIFICATION: dict[str, str] = {
    "결손금": (
        ...
        "(최보수 ×3.70 p=0.0002 / ×3.15 p=0.0002) · 다중검정 보정(가족 14) **Bonferroni "
        "까지 통과**"
    ),
    "영업손실": (
        "**채택(약)** · 14개 조합 전부에서 유의 (최보수 ×2.13 p=0.0077 / ×2.03 p=0.0124) "
        "이지만 다중검정 보정에서 **FDR 만 통과하고 Bonferroni 는 못 넘는다**(임계 0.00357). "
        "결손금보다 한 단계 약한 근거다"
    ),
    "공동의존점 근접":
...
def accumulated_deficit(retained: float | None, *, year: str = "") -> Flag | None:
    ...
    **배율을 "망한다" 로 읽으면 안 된다.** 기저율이 2.9% 라 ×3.7 은 6% 다 — 걸린
    기업의 94%는 2년 안에 아무 일도 없었다. 상장사의 25%가 여기 걸리고 실제 부실의
    절반만 잡는다. `signal/usefulness.py` 가 이 숫자를 낸다.
    ...
    return Flag(
        ...
        evidence=["결손 기업의 이후 2년 부실률 실측 5.2~6.1% "
                  ...
                  "다만 **결손 기업의 94%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 25%가 여기 걸리고, 실제 부실의 절반(49~52%)만 잡는다"],
    )
    ...
    **결손금보다 한 단계 약한 근거다.** 판정이 나온 검정 14개를 한 가족으로 묶어
    ...
    return Flag(
        ...
        evidence=["영업손실 기업의 이후 2년 부실률 실측 4.3~5.2% "
                  ...
                  "다만 **영업손실 기업의 95%는 2년 안에 아무 일도 없었다** — "
                  "상장사의 25%가 여기 걸리고, 실제 부실의 42~44%만 잡는다"],
    )
