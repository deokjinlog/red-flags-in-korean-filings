...
두 가지를 받는다:
  `elestock.json`     임원·주요주주 특정증권등 소유상황보고. **증감 칸이 있다**
    ...
    ⚠️ **그런데 `bsns_year` 를 무시한다.** 2019·2021·2023 을 각각 요청해도 같은 4건이
       오고 접수일이 전부 2024~2026 이다 — **현재 이력 스냅샷**이다. 과거 기준시점
       신호로 쓰면 2026년 보고를 2021년에 알았다고 세는 것이라 미래를 훔쳐본다.
       관리종목(KIND)과 같은 모양이고, 같은 이유로 **못 쓴다.**

       실측: T=2024-06-30 이전 접수 보고가 43,604건 중 **0건**. 처음엔 '내부자 매도
       해당 0사' 로 조용히 나왔고, 그게 신호가 없는 게 아니라 **애초에 시점 분리가
       불가능**하다는 뜻이었다.

       최대주주 변경(`hyslrChgSttus`)은 다르다 — `change_on` 이 실제 과거 날짜라
       시점 분리가 된다. 그쪽만 쓴다.
