...
### [2026-08-13 15:06] [코드-수정] (실데이터 1회전 교정)
- **id**: CH-20260813-010
- **이유**: `DART_API_KEY` 확보 후 실 API 1회전(삼성전자·SK하이닉스, 2024 사업보고서). **계약 fixture 로는 전혀 잡히지 않던 결함 5건**이 드러남. fixture 를 계획서의 가정대로 만들었기 때문에, 가정 자체가 틀렸다는 사실은 fixture 가 구조적으로 검증할 수 없었다 — 파서와 fixture 가 같은 오타를 공유하며 자기들끼리 일관된 허구를 이루고 통과하고 있었음
- **무엇이**:
  - **D1 `parse_investment` 지분율 필드** — `trmend_qota_rt` → **`trmend_blce_qota_rt`**. 실측: 삼성전자 138건 중 **0건** 파싱 / SK하이닉스 49건 중 **0건**. 올바른 필드로는 130건·46건. **교차확인과 모순검출이 통째로 무력화된 상태였음**
  - **D2 모순A 버킷 키에 `stock_knd` 추가** — 실측: SK하이닉스는 같은 지분을 `보통주` 20.07% 와 `의결권 있는 주식` 20.07% 로 **중복 게시**한다. 섞어 합산하면 40.14%. 지분율 50% 이상 기업에서는 100% 를 넘겨 **가짜 1급 모순**이 생성됨
  - **D3 `scope_key` 가 기준일만 사용** — 실측: FY2024 사업보고서의 `rcept_no` 는 `2025`로 시작(접수연도). 접수연도를 스코프 키에 섞으면 같은 기준일의 정정공시가 다른 버킷으로 갈라져 **진짜 불일치를 CHANGE 로 오분류**
  - **D5 (최대 위험) 지분율 대신 주식수 우선 비교** — `RelationEdge.share_qty` 신설, `compare_scope` 가 주식수를 우선 판정. 실측: 삼성전자↔삼성물산이 **동일한 298,818,100주**를 `5.01%`(보통주 분모)와 `4.4%`(총발행주식 분모)로 신고. 지분율만 비교하면 **우선주를 발행한 거의 모든 상장사에서 체계적 오탐**이 발생 — 제품의 간판 기능이 노이즈 생성기가 될 뻔했음
  - **D4 (코드 변경 없음, 설정 이슈)** — `induty_code` 가 3자리(삼성전자 `264`). 계획서 예시의 5자리 접두사 가정은 재보정 필요. 대상 선정은 어차피 수동 보정을 요구하므로 코드 결함은 아님
- **영향범위**: `parse/relation.py`(`stock_knd`·`share_qty` 신설) · `parse/structured_rel.py`(`parse_qty` 신설, 두 파서 필드 교정) · `trust/scope.py`(`scope_key`·`compare_scope`) · `trust/contradiction.py`(버킷 키) · `trust/crosscheck.py`(CONFLICT detail 에 주식수) · `tests/test_structured_rel_more.py`(fixture 오타 교정) · 계획서 Task 11 블록(동일 오타)
- **위험 카테고리**: `breaking 2`(엣지 스키마 필드 추가 · 스코프 키 의미 변경) · `side-effect 1`(모순 검출 건수 변동)
- **검증**: 실데이터 재검증에서 **CONFLICT 1건 → 0건, CONFIRMED 1건 → 2건** (삼성물산이 가짜 모순에서 올바른 일치로 재분류). 회귀 테스트 11건 추가, 전체 **118 passed / 0 failed**
- **연관 commit**: `669414d`
- **연관 항목**: CH-20260813-008

### [2026-08-13 15:06] [검증] (실데이터 1회전)
- **id**: CH-20260813-011
- **이유**: 슬라이스 1 의 실동작 확인 — 실 API → 파싱 → 해소 → 교차확인 → 모순검출 전 경로
- **무엇이**: 삼성전자(00126380) · SK하이닉스(00164779), 2024 사업보고서 기준
- **결과**: **PASS** — 파이프라인 전 구간 실동작 확인
  - DS002 엔드포인트 6종 전부 생존 (`hyslrSttus` · `hyslrChgSttus` · `otrCprInvstmntSttus` · `exctvSttus` · `accnutAdtorNmNdAdtOpinion` · `stockTotqySttus`)
  - `corpCode.xml` 118,706개 기업 확보
  - 삼성전자: 최대주주 26건 → 법인 주주 4명 해소 → 교차확인 **CONFIRMED 2 · CONFLICT 0 · SINGLE 24**
  - SK하이닉스: 최대주주 15건 → 법인 주주 1명 해소 → **SINGLE 15**
  - 모순 A: **0건** (양사 모두). AC-5 가 명시한 대로 **0건도 유효한 결과** — 검사가 돌았다는 것이 기준
- **미결 관찰**:
  - ⚠️ **해소율 29.7%** — 개인 주주(홍라희·이재용 등)는 `corp_code` 가 없어 미해소가 정상이나, 현재 지표는 이를 구분하지 않는다. 층1의 품질 게이트(AC-12)가 이 숫자를 그대로 임계로 쓰면 **정상 상태를 불합격 처리**한다. **자연인/법인 분리 집계가 필요**
  - ⚠️ **SINGLE 비율이 압도적**(24/26, 15/15) — 대부분 개인 주주라 반대편 신고가 애초에 존재하지 않는다. `crosscheck.py` 의 RISK 주석대로 SINGLE 을 신뢰도 근거로 쓰면 안 된다는 점이 실측으로 확인됨
  - ⚠️ 삼성물산의 반대편 신고 `rcept_no` 가 `20250814002350`(8월 접수)인데 기준일은 `2024-12-31`. D3 교정 덕분에 같은 스코프로 올바르게 매칭됨 — 교정 전이었다면 놓쳤을 사례
- **연관 commit**: `669414d`
- **연관 항목**: CH-20260813-010

### [2026-08-13 15:16] [코드-수정] (커버리지 확대 회전 교정)
- **id**: CH-20260813-012
- **이유**: 씨앗 24개 → 1홉 관계 확장 → 47사 전수 교차확인 실행. 1회전(2사)에서는 표본이 작아 안 보이던 **오탐 2종 15건**이 드러남. 업종코드 필터 대신 관계 확장을 쓴 이유는 (a) 상장사 3,982개에 `company.json` 전수 호출이 비싸고 (b) `induty_code` 가 3자리라 반도체 경계가 불분명하며 (c) **대조 쌍이 실제로 생기는 기업만** 모으는 게 목적이기 때문 — 요구사항 결정 2 의 "서로 얽힌 기업군" 에 직접 대응
- **무엇이**:
  - **D6 소계 행 제외** — `parse_major_shareholder` 가 `nm="계"` 요약 행을 주주 엣지로 만들고 있었다. 「최대주주 현황」 응답은 개별 주주 사이에 소계/합계 행(`stock_knd` 가 `보통주`/`기타`/`합계` 인 `계` 행)을 섞어 준다. 이를 함께 합산하면 지분 합계가 **정확히 두 배**가 된다. 실측: 티씨케이 50.4%+계 50.4%=100.8% · 삼성바이오로직스 74.35%×2=148.7% · 한미반도체 54.84%×2=109.68%. **모순A 8건 전부 이 원인**
  - **D7 주식수 반올림 단위 보정** — `compare_scope` 가 주식수를 정확 일치로 비교했다. 「타법인 출자현황」은 천주 단위로 반올림해 신고하는 경우가 있다(17,693,084 → 17,693,000). `reported_precision`/`qty_agrees` 신설: 거친 쪽의 신고 단위에 맞춰 양쪽을 반올림한 뒤 비교하며, 상한은 천주(`QTY_PRECISION_CAP`). 실측: 삼성 계열 **CONFLICT 7건 전부 이 원인** (차이 75~327주, 수천만 주 대비 0.001% 수준)
- **영향범위**: `parse/structured_rel.py`(`is_aggregate_row` 신설 + 파서 필터) · `trust/scope.py`(`reported_precision`·`qty_agrees` 신설, `compare_scope` 판정 교체)
- **위험 카테고리**: `side-effect 2` — 둘 다 모순 검출 건수를 직접 바꾼다. `_AGGREGATE_NAMES` 에 실제 주주명이 들어가면 그 주주가 통째로 누락되고, `QTY_PRECISION_CAP` 을 키우면 진짜 차이를 놓친다
- **검증**: 실데이터 재검증 — **CONFLICT 7 → 0 · 모순A 8 → 0 · CONFIRMED 13 → 20**(오탐 7건이 올바른 일치로 재분류) · 지분엣지 448 → 359(소계 89행 제거). 회귀 테스트 5건 추가, 전체 **123 passed / 0 failed**
- **연관 commit**: `8243138`
- **연관 항목**: CH-20260813-010, CH-20260813-013

### [2026-08-13 15:16] [검증] (커버리지 확대 회전)
- **id**: CH-20260813-013
- **이유**: "모순 0건이 **없어서**인가, **대조할 상대가 없어서**인가" 를 판정하기 위한 확대 측정
- **무엇이**: 씨앗 24개(해소 18) → 1홉 확장 → 상장 대상 **47사** · 지분엣지 359 · 출자엣지 957 · API 호출 94건
- **결과**: **PASS** — 파이프라인 정상, 그러나 **답은 "대조할 상대가 없어서"**
  - `CONFIRMED 20 · CONFLICT 0 · CHANGE 0 · SINGLE 339` · 모순A 0
  - **대조 가능 비율 5.6%** (359건 중 20건). 나머지 339건은 반대편 신고가 아예 존재하지 않음
  - **구조적 원인**: 최대주주 대부분이 자연인이고, **개인은 「타법인 출자현황」 신고 의무가 없다.** 법인 주주만 반대편이 존재한다
  - 대조 가능했던 20건 안에서는 모순 0 — 대기업 계열 신고는 실제로 정확했다
- **미사용 대조 축 (다음 수)**:
  - ⚠️ **지분공시 DS004(5% 대량보유상황보고) ↔ 최대주주현황** — **개인도 신고 의무가 있어** SINGLE 339건 중 상당수가 대조 가능해진다. `parse_major_holding` 은 슬라이스 1에서 **구현만 하고 수집 경로에 붙이지 않았음**. 요구사항 결정 2 의 "공시 종류를 넓히는 게 대조 쌍을 늘리는 것 — 노드는 덧셈, 이건 곱셈" 이 정확히 이 지점
  - 타법인출자 ↔ 상대사 「주식의 총수 현황」 (지분율 분모 검증)
- **누적 발견**: 실데이터가 드러낸 결함 **7건**(D1·D2·D3·D5·D6·D7 + D4 설정이슈). **계약 fixture 는 이 중 0건을 잡았다** — fixture 를 계획서의 가정대로 만들었으므로, 가정 자체가 틀렸다는 사실은 구조적으로 검증 불가능했음
- **연관 commit**: `8243138`
- **연관 항목**: CH-20260813-012

### [2026-08-13 15:32] [코드-수정] (합산 대조 축 신설)
- **id**: CH-20260813-014
- **이유**: CH-013 이 남긴 병목 해소 — 개별 주주 축은 **법인만** 대조 가능해 5.6% 에 그쳤다(개인은 「타법인 출자현황」 신고 의무 없음). 「대량보유 상황보고」는 **개인도 보고 의무**가 있고 그 값이 본인+특별관계자 합산이므로 「최대주주 현황」의 `계` 행과 짝이 된다. CH-012 에서 `계` 행을 주주 엣지에서 제외했는데, 제외한 그 행이 **다른 종류의 사실**로서 이 축의 한쪽 짝이었다
- **무엇이**: `trust/aggregate.py` 신설 · `parse_major_holding` 필드 교정(**D8**) · `tests/test_aggregate.py` 8건
  - **D8** — `stkqy_irds_rt` 는 실제 응답에 없는 키. 실측은 `stkrt`(보유비율)·`stkqy`(보유주식등의 수). fixture 가 같은 오타를 공유해 통과하고 있었다 (오늘 **8번째 동일 패턴**)
- **실데이터가 연속으로 깬 가정 3개** (회차마다 재설계):
  1. *대량보유보고 ≈ 개별 주주 지분* → **본인+특별관계자 합산**이라 4배 차이(삼성물산 20.08% vs 개별 5.01%)
  2. *아무 보고나 최신 것과 비교* → 국민연금·외국계 운용사도 각자 5% 보고를 낸다. 보고자를 안 가려 **다른 그룹끼리 비교**
  3. *최대주주 본인 = 대표보고자* → 삼성전자는 **본인 삼성생명(8.51%), 보고자 삼성물산(5.01%)**. 본인 이름 매칭은 정상 사례를 놓친다 → **특별관계자 명단 소속 여부**로 판정
  - 추가: 「대량보유 상황보고」의 '주식등' 은 종류 통합이라 **보통주 계 행만** 대상
- **영향범위**: 신규 모듈. 기존 축(`crosscheck.py`)과 독립 — 개별 주주 대조는 그대로고 이 축이 개인 보고자를 커버한다
- **위험 카테고리**: `side-effect 2` — `pct_tolerance`(시차 흡수)와 특별관계자 명단 매칭이 검출 건수를 직접 좌우
- **검증**: 실측 **CONFLICT 15 → 1**, 대조가능 53.8% → 35.7%(무관 짝 제거로 분모가 정직해짐), **131 passed**
- **연관 항목**: CH-20260813-013, CH-20260813-015

### [2026-08-13 15:32] [검증] (합산 대조 축 실데이터 회전)
- **id**: CH-20260813-015
- **이유**: 새 축이 실제로 모순을 잡는지, 잡은 것이 진짜인지 판정
- **무엇이**: 47사 · 계 행 42건 · 대량보유보고 476건 · 보고자 60명 (국민연금·정지완·삼성물산·하나마이크론·미래에셋 등 — **개인 보고자가 실제로 포함됨**)
- **결과**: `CONFIRMED 14 · CONFLICT 1 · NO_REPORT 27` — **진짜 위반 0건**
  - 유일한 CONFLICT(동진쎄미켐)도 검증 결과 **정당한 차이**였다
    - 회사 신고 계 19,857,824주(38.62%) vs 동진홀딩스 보고 17,977,824주(34.97%) · 차이 **1,880,000주**
    - 시차 아님 — 동진홀딩스는 2024-12-06 ~ 2025-12-04 동일 수치를 **일관되게** 신고
    - **차이가 「재단법인 동진장학연구재단」 보유분(1,880,000주)과 정확히 일치.** 재단이 「최대주주 현황」 특수관계인에는 포함되고 「대량보유 상황보고」 특별관계자에는 미포함 — **두 공시의 근거 법령상 범위 차이**이지 위반이 아니다
- **제품 요구사항 도출 (실증)**: CONFLICT 를 "오류" 로 단정하면 안 된다. **차이의 출처를 분해해 "어느 주주가 한쪽에만 있는가" 를 함께 제시**해야 한다. 있으면 판정 5초, 없으면 원문 추적. → **층2 감사 워크벤치의 필수 기능**
- **신호 대 잡음**: 합산 사실 42건 중 **1건만** 사람 검토 대상으로 올렸고, 그 1건은 근거를 펼치자 즉시 설명됐다. 검출기로서 적정 수준
- **연관 항목**: CH-20260813-014
